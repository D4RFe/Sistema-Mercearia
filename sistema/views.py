...
def relatorios(request):
    ...
    if request.method == "POST":
        ...
        elif tipo_relatorio == "relatorio-mercadorias":
            relatorios_mercadorias = NotaDeMercadoria.objects.filter(data__date__range=(data_inicial, data_final))
            lista_relatorio_mercadorias = [(relatorio.data.date(), relatorio.valor, relatorio.fornecedor.nome_empresa, relatorio.pk) for relatorio in relatorios_mercadorias]


    return render(request, "relatorios.html", {"lista_relatorios_venda": lista_relatorios_venda, "lista_relatorios_caixa": lista_relatorios_caixa , "lista_relatorios_mercadorias": lista_relatorio_mercadorias})
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.method == "POST":
            ...
            if button_type == "edit":
                ...
                produto.quantidade = quantidade
                produto.quantidade_minima_para_pedido = quantidade_minima
                produto.quantidade_minima_para_pedido = quantidade_minima
                produto.save()
        ...
        if button_type == "atualizarCliente":
            try:
                ...
                if len(cpf_cliente) == 11:
                    ...
                    cnpj_obj = CNPJ()
                    if not cnpj_obj.validate(cpf_cliente):
                        messages.error(request, "CNPJ inválido! Por favor, insira um CNPJ válido.")
                        return redirect('sistema:clientes')
                else:
                    messages.error(request, "Por favor insira um documento válido com 11 ou 14 dígitos!")
                    return redirect('sistema:clientes')
...
class RelatorioDeFornecedor(DetailView):
    template_name = "relatorioFornecedor.html"
    model = NotaDeMercadoria
    ...
    def get_context_data(self, **kwargs):
        ...
        date = relatorio.data.date()
        context['data'] = date
        context['itens'] = relatorio.itens.select_related('produto')
